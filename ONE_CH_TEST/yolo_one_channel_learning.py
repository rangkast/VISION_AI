...
# YOLO 모델 학습 함수
def train_yolo_model(data_yaml, model_path, epochs=50, batch_size=16, learning_rate=0.01, img_size=(640, 640)):
...
if __name__ == '__main__':
    ...
    # YOLO 모델 학습 (채널 1로 설정된 모델 사용)
    model_yaml = 'yolov10s.yaml'  # 모델 설정 파일 경로
    ...
    # # 기존 채널 설정을 ch: 1로 변경
    # model_config = model_config.replace("ch: 3", "ch: 1")
    # with open(model_yaml, 'w') as file:
    #     file.write(model_config)

    # 모델 학습
    train_yolo_model(yaml_path, model_path=model_yaml, epochs=50, batch_size=4, learning_rate=0.001, img_size=(640, 640))
